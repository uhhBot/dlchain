...
func (consensus *Consensus) didReachPrepareQuorum() error {
	...
	// baseline交易
	if consensus.DivisionLevel == 1 {
		txnsCnt := 0
		for _, item := range consensus.Blockchain.CurrentHeader().TransactionMsg() {
			txnsCnt += int(item.Count)
		}
		if txnsCnt != 0 {
			utils.Logger().Info().Int("number of txns", txnsCnt).Msg("add txns msg into body")
			...
			preMsg[3] = 3
			buf := make([]byte, 8)
			binary.BigEndian.PutUint64(buf, uint64(txnsCnt))
			shit := make([]byte, txnsCnt*(512)-12)
			shit = append(buf, shit...)
			shit = append(preMsg, shit...)
			msgToSend = append(shit, msgToSend...)
		}
	}
