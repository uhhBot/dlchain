...
func (consensus *Consensus) onPrepared(recvMsg *FBFTMessage) {
	...
	go func() {
		...
		for _, committedMsg := range consensus.FBFTLog.GetNotVerifiedCommittedMessages(blockObj.NumberU64(), blockObj.Header().ViewID().Uint64(), blockObj.Hash()) {
			if committedMsg != nil {
				tmp := false
				consensus.onCommitted(committedMsg, &tmp)
				// if tmp == 233 {
				// 	consensus.getLogger().Info().
				// 		Msg("uhhBot here changed to 233 leader")
				// }
			}
...
func (consensus *Consensus) onCommitted(recvMsg *FBFTMessage, divisionFlag *bool) {
	consensus.mutex.Lock()
	defer consensus.mutex.Unlock()
	...
	// uhhBot 投票人的调用
	consensus.tryCatchup(divisionFlag)
