...
type Consensus struct {
	// 我加的
	Index uint32 // 代表节点在list中的标号 用于后续增加分片

	DivisionLevel uint32
	MetaGroupID   uint32
	MetaGroupSize uint32
	...
	// calculate cross-shard
	SeenMap map[string]bool
	// 这个最早是用来设计处理跨分片事务的
	NewTxnsForNextBlock []blockif.TransactionMsg
	// 给上面这个狗东西上个锁
	NewTxnsMutex sync.Mutex
	...
	LastConsensusReachedTime time.Time

	WaitTime             int
	WaitTimeMeta         int
	StopMainFlag         int
	FirstTransactionTime uint64
	BlockTxsCnt          int
}
...
func New(
	...
	Decider quorum.Decider, waitTime int, waitTimeMeta int, stopMainFlag int,
) (*Consensus, error) {
	consensus := Consensus{}
	consensus.Decider = Decider
	consensus.host = host
	consensus.WaitTime = waitTime
	consensus.WaitTimeMeta = waitTimeMeta
	consensus.StopMainFlag = stopMainFlag
	consensus.FirstTransactionTime = 4
	consensus.BlockTxsCnt = 4096
