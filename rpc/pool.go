...
func (s *PublicPoolService) SendRawTransaction(
	...
	go func() {
		...
		if err := s.hmy.SendTx(ctx, tx); err != nil {
			utils.Logger().Warn().Err(err).Msg("uhhBot Could not submit transaction")
			// return txHash, err
		}
	}()
	...
	if tx.To() == nil {
		...
		utils.Logger().Info().
			...
			Interface("tx", tx).
			// Str("nonce", string(tx.Nonce())).
			// Str("gasPrice", tx.GasPrice().String()).
			// Str("shardID", string(tx.ShardID())).
			// Str("toShardID", string(tx.ToShardID())).
			// Str("to", tx.To().String()).
			// Str("value", tx.Value().String()).
			Msg("Submitted transaction ouo")
	}
