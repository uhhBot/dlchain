...
var (
	...
	waitTimeFlag = cli.IntFlag{
		Name:     "waiting_time",
		Usage:    "wdnmd",
		DefValue: 10,
	}
	waitTimeMetaFlag = cli.IntFlag{
		Name:     "waiting_time_meta",
		Usage:    "wdnmd meta",
		DefValue: 1,
	}

	stopMainFlag = cli.IntFlag{
		Name:     "stop_main_flag",
		Usage:    "wdnmd main stop",
		DefValue: 0,
	}
)
...
func applyGeneralFlags(cmd *cobra.Command, config *harmonyconfig.HarmonyConfig) {
	...
	if cli.IsFlagChanged(cmd, waitTimeFlag) {
		config.General.WaitingTime = cli.GetIntFlagValue(cmd, waitTimeFlag)
	}
	if cli.IsFlagChanged(cmd, waitTimeMetaFlag) {
		config.General.WaitingTimeMeta = cli.GetIntFlagValue(cmd, waitTimeMetaFlag)
	}
	if cli.IsFlagChanged(cmd, stopMainFlag) {
		config.General.StopMainFlag = cli.GetIntFlagValue(cmd, stopMainFlag)
	}
