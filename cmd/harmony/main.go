...
func createGlobalConfig(hc harmonyconfig.HarmonyConfig) (*nodeconfig.ConfigType, error) {
	...
	nodeConfig.Index = initialAccounts[0].AccIndex
	...
	nodeConfig.WaitingTime = hc.General.WaitingTime
	nodeConfig.WaitingTimeMeta = hc.General.WaitingTimeMeta
	nodeConfig.StopMainFlag = hc.General.StopMainFlag
...
func setupConsensusAndNode(hc harmonyconfig.HarmonyConfig, nodeConfig *nodeconfig.ConfigType) *node.Node {
	...
	currentConsensus, err := consensus.New(
		myHost, nodeConfig.ShardID, p2p.Peer{}, nodeConfig.ConsensusPriKey, decider, nodeConfig.WaitingTime, nodeConfig.WaitingTimeMeta, nodeConfig.StopMainFlag,
	)
	currentConsensus.DivisionLevel = 1
	currentConsensus.Index = uint32(nodeConfig.Index)
	currentConsensus.SeenMap = make(map[string]bool)
	// magic 最多会有十级
	currentConsensus.MetaStage = make([]consensus.MetaStage, 15)

	for index, _ := range currentConsensus.MetaStage {
		currentConsensus.MetaStage[index].DivisionLevel = uint32(index)
		currentConsensus.MetaStage[index].BlockID = make([]uint32, index)
	}
	...
	currentNode := node.New(myHost, currentConsensus, chainDBFactory, blacklist, nodeConfig.ArchiveModes(), &hc, nodeConfig.Index)
	...
	currentNode.SeenReceipt = make(map[ethCommon.Hash]bool)
