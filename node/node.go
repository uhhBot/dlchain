...
type Node struct {
	Index                 int                               // 代表节点在list中的标号 用于后续增加分片
	...
	// uhhBot control transactions
	Ks            *keystore.KeyStore
	Acct          *accounts.Account
	FirstTxViewID uint64

	// uhhBot test
	SeenReceipt map[common.Hash]bool
}
...
func (node *Node) addPendingTransactions(newTxs types.Transactions) []error {
	...
	// 	utils.Logger().
	// 		Err(errs[0]).
	// 		Msg("uhhBot oh no dding more transactions")
	// }
	utils.Logger().Info().
		Interface("err", errs).
		Int("err length", len(errs)).
		Int("length of newTxs", len(newTxs)).
		Int("totalPending", pendingCount).
		Int("totalQueued", queueCount).
		Int("nonce", int(newTxs[0].Nonce())).
		Int("size", int(newTxs[0].Size())).
...
// baseline 跨分片第二段？
func (node *Node) AddPendingReceiptsDIY(receipt *types.CXReceiptsProofDIY) {
	node.pendingCXMutex.Lock()
	defer node.pendingCXMutex.Unlock()

	// 确认这个跨分片交易是否处理过，如果没有则包含到区块里面去，记录一下数量和所花费的时间
	// map的key {fromShard, blockID}
	// 在node的consensus里面存一个map
	key := strconv.Itoa(int(receipt.Header.ShardID())) + receipt.Header.Number().String()
	_, ok := node.Consensus.SeenMap[key]
	if ok {
		return
	} else {
		node.Consensus.SeenMap[key] = true

		node.Consensus.NewTxnsForNextBlock = append(node.Consensus.NewTxnsForNextBlock, blockif.TransactionMsg{
			FromShard: receipt.Header.ShardID(),
			ToShard:   node.Consensus.ShardID,
			Count:     receipt.Header.TransactionMsg()[node.Consensus.ShardID-1].Count,
			BeginTime: uint32(receipt.Header.Time().Uint64()),
		})
	}
}
...
func (node *Node) AddPendingReceipts(receipts *types.CXReceiptsProof) {
	...
	// DDoS protection
	const maxCrossTxnSize = 4096000000
...
func (node *Node) validateNodeMessage(ctx context.Context, payload []byte) (
	...
	case proto_node.Block:
		...
		case proto_node.ReceiptDIY:
			...
			msgPayload := payload[p2pNodeMsgPrefixSize+1:]
			cxp := types.CXReceiptsProofDIY{}
			if err := rlp.DecodeBytes(msgPayload, &cxp); err != nil {
				utils.Logger().Error().Err(err).
					Msg("[ProcessReceiptMessageDIY] Unable to Decode message Payload")
				nodeNodeMessageCounterVec.With(prometheus.Labels{"type": "invalid_node_type"}).Inc()
				return nil, 0, errInvalidNodeMsg
			}

			_, ok := node.SeenReceipt[cxp.Header.Hash()]
			if ok {
				nodeNodeMessageCounterVec.With(prometheus.Labels{"type": "invalid_block_type"}).Inc()
				utils.Logger().Info().Str("Hash", cxp.Header.Hash().String()).
					Msg("[preProcessReceiptMessageDIY] see old receipt,")

				return nil, 0, errInvalidNodeMsg
			} else {
				node.SeenReceipt[cxp.Header.Hash()] = true
				utils.Logger().Info().Str("Hash", cxp.Header.Hash().String()).
					Msg("[preProcessReceiptMessageDIY] see new receipt,")
			}

			nodeNodeMessageCounterVec.With(prometheus.Labels{"type": "node_receipt_diy"}).Inc()
			// 我改了，增加对contract相关的验证
		case proto_node.HoriConsensus:
			nodeNodeMessageCounterVec.With(prometheus.Labels{"type": "HoriConsensus"}).Inc()
			// 我改了，增加对contract相关的验证
		case proto_node.HoriBlockHeader:
			// uhhBot test 不要收那么多metablock
			nodeNodeMessageCounterVec.With(prometheus.Labels{"type": "horiblock header"}).Inc()
		...
		if err := pubsub.RegisterTopicValidator(
			...
			func(ctx context.Context, peer libp2p_peer.ID, msg *libp2p_pubsub.Message) libp2p_pubsub.ValidationResult {
				...
				if openBox[0] == 0 &&
					...
					openBox[2] == 2 &&
					openBox[3] == 3 {
					numTxns := int64(binary.BigEndian.Uint64(openBox[4:12]))
					openBox = openBox[numTxns*512:]
				}
					...
					go func() {
						...
						if semConsensus.TryAcquire(1) {
							...
							if isThisNodeAnExplorerNode {
								...
								divisionFlag := false
								if err := msg.handleC(ctx, msg.handleCArg, msg.senderPubKey, &divisionFlag); err != nil {
									errChan <- withError{err, msg.senderPubKey}
								}
								if divisionFlag {
									// 暂时不需要退出旧的
									// if node.Consensus.DivisionLevel > 2 {
									// 	pubsub := node.host.PubSub()
									// 	utils.Logger().Info().Msg("uhhBot uhhBot you now, we are going to leave the group ")
									// 	pubsub.UnregisterTopicValidator(node.Consensus.MetaGroupName)
									// }

									// 加入新的
									utils.Logger().Info().Msg("uhhBot 分裂")
									node.AddOneMoreGroup(nodeconfig.GroupID(node.Consensus.MetaGroupName))
									// 发个东西呗
									if node.Consensus.IsMetaLeader {
										// 临时
