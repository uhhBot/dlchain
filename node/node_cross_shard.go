...
func (node *Node) BroadcastCXReceipts(newBlock *types.Block) {
	...
	for i := 1; i < shardNum; i++ {
		...
		node.BroadcastCXReceiptsWithShardIDDIY(newBlock, uint32(i))
	}
}

// BroadcastCXReceiptsWithShardID broadcasts cross shard receipts to given ToShardID
func (node *Node) BroadcastCXReceiptsWithShardIDDIY(block *types.Block, toShardID uint32) {
	myShardID := node.Consensus.ShardID
	utils.Logger().Debug().
		Uint32("toShardID", toShardID).
		Uint32("myShardID", myShardID).
		Uint64("blockNum", block.NumberU64()).
		Interface("TRANSACTION", block.Header().TransactionMsg()).
		Msg("[BroadcastCXReceiptsWithShardID]")

	if len(block.Header().TransactionMsg()) == 0 || block.Header().TransactionMsg()[toShardID-1].Count == 0 {
	...
	// 一个缩略的交易大小 80
	size := 80 * int(block.Header().TransactionMsg()[toShardID-1].Count)
	payload := make([]byte, size)
	cxReceiptsProofDIY := &types.CXReceiptsProofDIY{
		Header:  block.Header(),
		Payload: payload,
	}
	...
	utils.Logger().Info().Uint32("ToShardID", toShardID).
		...
		// Interface("cxp", cxReceiptsProofDIY).
		Msg("[BroadcastCXReceiptsWithShardID] ReadCXReceipts and MerkleProof ready. Sending CX receipts...")
	// TODO ek – limit concurrency
	node.host.SendMessageToGroups([]nodeconfig.GroupID{groupID},
		p2p.ConstructMessage(proto_node.ConstructCXReceiptsProofDIY(cxReceiptsProofDIY)),
	)
}
...
// ProcessReceiptMessageDIY store the receipts and merkle proof in local data store
func (node *Node) ProcessReceiptMessageDIY(msgPayload []byte) {
	...
	utils.Logger().Debug().
		Msg("[ProcessReceiptMessageDIY] Add CXReceiptsProofDIY to pending Receipts")
	// TODO: integrate with txpool
	// node.AddPendingReceipts(&cxp)
	// do something
	node.AddPendingReceiptsDIY(&cxp)
}
