...
func (w *Worker) FinalizeNewBlock(
	...
	crossLinks types.CrossLinks, shardState *shard.State, DivisionLevel uint32, BlockIDs []uint32, CrossTxnsMsg []blockif.TransactionMsg,
) (*types.Block, error) {
	...
	// 临时 动态交易
	// 如果已经分了，那么只放小区块的元数据，否则塞入具体的交易
	if w.GetCurrentHeader().Number().Int64() == 3 {
		w.LastTime = time.Now()
	}
	if DivisionLevel != 1 {
		w.current.header.SetDivisionLevel(DivisionLevel)
		tmpIDs := make([]uint32, len(BlockIDs))
		for i, _ := range BlockIDs {
			tmpIDs[i] = BlockIDs[i]
		}
		w.current.header.SetBlockIDs(tmpIDs)
		// uhhBot test 临时用于gear
	} else if w.GetCurrentHeader().Number().Int64() > 3 {
		utils.Logger().Debug().Msg("uhhBot baseline交易")
		// 不用加这么多 uhhBotTODO
		ins := shard.Schedule.InstanceForEpoch(common.Big0)
		numShards := ins.NumShards()
		tmpTXNS := make([]blockif.TransactionMsg, numShards-1)
		// 这儿不是4096 而是看情况的结果
		// TotalTXNCNT := uint32(time.Since(w.LastTime).Seconds() * 150)
		w.LastTime = time.Now()
		TotalTXNCNT := uint32(4096)
		for i := 0; i < int(numShards-1); i++ {
			tmpTXNS[i].FromShard = uint32(w.chain.ShardID())
			tmpTXNS[i].ToShard = uint32(i + 1)
			tmpTXNS[i].Count = uint32(TotalTXNCNT / (numShards - 1))
			// 稍加一点交易
			// 自己的用不上
			// 这儿时间是不是可以直接设置了？
		}
		w.current.header.SetTransactionMsg(tmpTXNS)
	}
	w.current.header.SetTransactionReceiptMsg(CrossTxnsMsg)
...
func New(
	...
	worker.gasFloor = 800000000
	worker.gasCeil = 1200000000
