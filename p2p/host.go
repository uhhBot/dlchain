...
func NewHost(cfg HostConfig) (Host, error) {
	...
	// 修复双重nat问题，获得环境变量的ip
	externalIP := os.Getenv("sysExternalIP")
	var extMultiAddr ma.Multiaddr
	if externalIP == "" {
		utils.Logger().Debug().
			Msg("uhhBot oh no External IP not defined, Peers might not be able to resolve this node if behind NAT")
	} else {
		extMultiAddr, err = ma.NewMultiaddr(fmt.Sprintf("/ip4/%s/tcp/%s", externalIP, self.Port))
		if err != nil {
			utils.Logger().Err(err).
				Msg("uhhBot oh no Error creating multiaddress")
			return nil, err
		}
	}
